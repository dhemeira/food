...
import { firebaseConfig, imageApiUrl } from './firebase/config';
...
export type { CalorieUnit, Ingredient, Recipe, RecipeInput, Role, Step, User } from './types';
export { CALORIE_UNITS } from './types';

const { auth, firestore } = initFirebase(firebaseConfig);

export const backend: Backend = new FirebaseBackend(auth, firestore, imageApiUrl);
